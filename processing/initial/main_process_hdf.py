...
Main script for extracting behavioral data and related decoder information for each BCI session
    ...
    Hard-coded path to save pickles (pickle_path)
    ...
    For each date, a separate .pkl file is saved for df and dfK in the pickles folder.
        ...
        subj: str, four-letter code for subject 
        mode: str, BCI perturbation type
        date: str, date of session in format YYYYMMDD 
    ...
    is saved that contains all of the dates and perturbation degrees.
        dDates, dDegs, dDegs2 || dDates_dDegs_HDF_{mode}.pkl
...
def run_main_process_hdf(root_path, dRoot, pickle_path, modes, dSubject):
    ...
    for mode in modes: 
        ...
        for subject_ind in range(len(dSubject)): #[0,1]:
            
            
            subj, subject = dSubject[subject_ind]#in zip([0,1],['airp', 'braz'],['Airport', 'Brazos']):
