...
for i in [0,1]:
    ...
    dfPARAMS = dRES[i]
    dfSIG_ = dfPARAMS.loc[(dfPARAMS['shuff']==True) & (dfPARAMS['sig']==1)]
    
    
    for q, DEG in zip([0,1],[50,90]):
        
        dBS[i][DEG] = {}
        
        color = palette_ROT[DEG]

        dfSIG = dfSIG_.loc[(dfSIG_['a_dPD']==DEG) | (dfSIG_['a_dPD']==-1*DEG)]
        ...
        for j,KEY in enumerate(['svBL', 'KY']):
            ...
            if i == 0:
                ...
                alpha=0.03
            axis.scatter(xdata,ydata, color=color, alpha=alpha, zorder=0)
            params, perr, tvalues, pvalues = fit_linear_model(xdata, ydata, model_type='L1')        
            ...
            m,b = params
            print(f'{subject} | {DEG}, {KEY}| m={m:.2f}, b={b:.2f}')
...
legend_elements = [Line2D([0],[0], marker='s', markersize=8, color=orange, lw=0, label='easy'),
                   Line2D([0],[0], marker='s', markersize=8, color=purple, lw=0, label='hard')]

legend = ax[0][0].legend(handles=legend_elements, ncol=1, columnspacing=0.3, handletextpad=0.0, loc=(0.72,0.825), fontsize=fontsize-2)
...
for i in [0,1]:
    ...
    for q, DEG in zip([0,1],[50,90]):
        ...
        for j,KEY in enumerate(['svBL', 'KY']):
            ...
            axis.hist(rs, bins=bins, color=color, edgecolor='w', lw=0.5, alpha=1, density=True)

            axis.scatter(np.mean(rs), 18, color=color, marker=marker, s=75)
            
            if  (KEY=='KY') and (DEG==90):
                q_adj = 0.05
            else:
                q_adj = 0
            
            if  (KEY=='KY') and (DEG==50):
                q_adj = -0.05
            else:
                q_adj = 0
            axis.text(np.mean(rs)+q_adj, 19, f'{np.mean(rs):.2f}', ha='center', va='bottom', color=color, fontsize=fontsize)
    ...
    for j,KEY in enumerate(['svBL', 'KY']):
        ...
        axis.plot([np.mean(v1),np.mean(v2)], [18,18],color='k', zorder=0)


ax[0][0].set_ylabel('density', fontsize=fontsize+2)
ax[1][0].set_ylabel('density', fontsize=fontsize+2)       
ax[1][0].set_xlabel('r$^2$', fontsize=fontsize+2)   
ax[1][1].set_xlabel('r$^2$', fontsize=fontsize+2)   

ax[0][0].text(-0.95,y_loc-2,'ln(%sv)', fontsize=fontsize+2) 
ax[0][1].text(-0.95,y_loc-2,'ln(%sv)', fontsize=fontsize+2) 

ax[1][0].text(-0.95,y_loc-2,'ln(%contribution)', fontsize=fontsize+2) 
ax[1][1].text(-0.95,y_loc-2,'ln(%contribution)', fontsize=fontsize+2) 



legend_elements = [Line2D([0],[0], marker='d', markersize=8, color=orange, lw=0, label='easy'),
                    Line2D([0],[0], marker='D', markersize=8, color=purple, lw=0, label='hard')]

legend = ax[0][0].legend(handles=legend_elements, ncol=1, columnspacing=0.3, handletextpad=0.0, loc=(0.75,0.85), fontsize=fontsize-1)
